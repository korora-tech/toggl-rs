...
fn test_get_workspace_goals() -> Result<()> {
    ...
    with_mockito(
        ...
        Some(response),
        |client| {
            let goals = client.goals().get_workspace_goals(111, None)?;
            assert_eq!(goals.len(), 1);

            let goal = &goals[0];
            assert_eq!(goal.id, 123);
...
fn test_get_workspace_goals_with_query() -> Result<()> {
    ...
    with_mockito(
        ...
        |client| {
            ...
            let goals = client.goals().get_workspace_goals(111, Some(query))?;
...
fn test_create_workspace_goal() -> Result<()> {
    ...
    with_mockito(
        ...
        Some(response),
        |client| {
            let goal = client.goals().create_workspace_goal(111, request)?;
            assert_eq!(goal.id, 124);
...
fn test_get_workspace_goal() -> Result<()> {
    ...
    with_mockito(
        ...
        Some(response),
        |client| {
            let goal = client.goals().get_workspace_goal(111, 123)?;
            assert_eq!(goal.id, 123);
            assert_eq!(goal.name, "Weekly coding goal");
            Ok(())
        },
...
fn test_update_workspace_goal() -> Result<()> {
    ...
    with_mockito(
        ...
        Some(response),
        |client| {
            let goal = client.goals().update_workspace_goal(111, 123, request)?;
            assert_eq!(goal.id, 123);
...
fn test_delete_workspace_goal() -> Result<()> {
    with_mockito(
        ...
        None,
        |client| {
            client.goals().delete_workspace_goal(111, 123)?;
            Ok(())
        },
    )
}

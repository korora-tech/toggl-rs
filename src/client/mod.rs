...
impl TogglClient {
    ...
    pub fn new_with_base_url(api_token: String, base_url: &str) -> Result<Self> {
        let client = Client::builder()
            .build()
            .map_err(|e| Error::LibraryError(format!("Failed to create HTTP client: {}", e)))?;
    ...
    pub(crate) fn request_binary(&self, method: Method, path: &str) -> Result<Vec<u8>> {
        ...
        if status.is_success() {
            response
                .bytes()
                .map(|b| b.to_vec())
                .map_err(|e| Error::LibraryError(format!("Failed to read response bytes: {}", e)))
        } else {
            let error_text = response
                .text()
                .unwrap_or_else(|_| "Unknown error".to_string());
    ...
    pub(crate) fn request_multipart_with_mime(
        ...
        let part = reqwest::blocking::multipart::Part::bytes(file_data.to_vec())
            ...
            .mime_str(mime_type)
            .map_err(|e| Error::LibraryError(format!("Failed to create multipart: {}", e)))?;
        ...
        let response = self
            ...
            .multipart(form)
            .send()
            .map_err(|e| Error::LibraryError(format!("Failed to send request: {}", e)))?;
    ...
    pub(crate) fn request_multipart_json_file<T: DeserializeOwned, B: Serialize>(
        ...
        let json_string = serde_json::to_string(json_data)
            .map_err(|e| Error::LibraryError(format!("Failed to serialize JSON: {}", e)))?;

        // Create multipart form with both JSON and file
        let json_part = reqwest::blocking::multipart::Part::text(json_string)
            .mime_str("application/json")
            .map_err(|e| Error::LibraryError(format!("Failed to create JSON part: {}", e)))?;

        let file_part = reqwest::blocking::multipart::Part::bytes(file_data.to_vec())
            .file_name(file_name.to_string())
            .mime_str(mime_type)
            .map_err(|e| Error::LibraryError(format!("Failed to create file part: {}", e)))?;
        ...
        if response.status().is_success() {
            response
                .json::<T>()
                .map_err(|e| Error::LibraryError(format!("Failed to deserialize response: {}", e)))
        } else {
            let status = response.status();
            let error_text = response
                .text()
    ...
    fn handle_response<T: DeserializeOwned>(&self, mut response: Response) -> Result<T> {
        ...
        if status.is_success() {
            response
                .json::<T>()
                .map_err(|e| Error::LibraryError(format!("Failed to deserialize JSON: {}", e)))
        } else {
            let error_text = response
                .text()
                .unwrap_or_else(|_| "Unknown error".to_string());
